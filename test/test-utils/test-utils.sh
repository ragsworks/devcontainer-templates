...
#               Microsoft's Dev Container Specification. (see
...
{
	if [ -z "$1" ]; then
	    PACKAGE_LIST="apt-utils git openssh-client less iproute2 procps curl wget unzip nano jq lsb-release ca-certificates \
    		apt-transport-https dialog gnupg2 libc6 libgcc1 libgssapi-krb5-2 liblttng-ust0 libstdc++6 zlib1g locales sudo"
    else
    	PACKAGE_LIST="$1"
    fi
